...
final as (
        ...
        select 
            ...
            sum(case 
                when orders.order_status not in ('returned','return_pending') then payment_amount else 0 
                end)/nullif(count(case when orders.order_status not in ('returned','return_pending') then 1 
                end),0)
